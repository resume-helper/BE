...
| # | 논의 포인트 | 결정 |
|---|------------|------|
| 1 | 블록 저장 단위 | 일괄 저장 (FE가 전체 상태 일괄 전송) |
| 2 | 블록 버전 관리 전략 | block_versions 미구현, 버전 스냅샷 전체 보류 |
| 3 | resume_blocks 필요 여부 | 유지 (실시간 편집 구성 분리) |
| 4 | reviewer_ip 저장 방식 | 원문 저장 VARCHAR(45) |
| 5 | notifications 위치 | MVP에서 테이블 전체 제거, Phase 2 이후 추가 |
...
| 항목 | 내용 |
|------|------|
| **결정** | block_versions 미구현, 버전 스냅샷(resume_versions) 전체 보류 |
| **이전 결정** | block_versions 별도 테이블 (2026-04-30, 재검토 후 변경) |
| **이유** | MVP 범위 내 복잡도 감소. 단순 CRUD 우선 구현 후 Phase 2에서 추가 |
| **결정일** | 2026-05-07 |
...
| 항목 | 내용 |
|------|------|
| **결정** | 랜덤 UUID 자동 생성, 사용자 지정 미지원 (MVP 이후 검토) |
| **이유** | 간단하고 충돌 없음. Short UUID나 사용자 지정은 MVP 이후 검토 |
| **결정일** | 2026-05-07 |
...
| 10 | 슬러그 구조 | 랜덤 UUID 자동 생성, 사용자 지정 미지원 | 2026-05-07 |
| 11 | Redis HA | 단일 Redis (JWT Refresh Token 관리 전용) | 2026-04-30 |
| 2 | ERD 설계 | DDL 확정 (doc/erd_cloud_import.sql), 세부 결정 사항은 [2] 참조 | 2026-05-07 |
